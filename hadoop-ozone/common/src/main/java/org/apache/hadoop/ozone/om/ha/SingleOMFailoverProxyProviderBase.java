...
public abstract class SingleOMFailoverProxyProviderBase<T> implements FailoverProxyProvider<T> {
  ...
  private InetSocketAddress omNodeAddress;
  ...
  public SingleOMFailoverProxyProviderBase(ConfigurationSource configuration, UserGroupInformation ugi,
    ...
    Preconditions.checkNotNull(omNodeAddress);
  }
  ...
  public RetryPolicy getRetryPolicy(int maxRetries) {
    RetryPolicy retryPolicy = new RetryPolicy() {
      ...
      public RetryAction shouldRetry(Exception exception, int retries,
        ...
        if (exception instanceof ServiceException) {
          ...
          if (leaderNotReadyException != null) {
            ...
            // incremented
            return getRetryAction(RetryDecision.RETRY, failovers);
          }
        }
      ...
      private RetryAction getRetryAction(RetryDecision fallbackAction, int failovers) {
        if (failovers < maxRetries) {
          return new RetryAction(fallbackAction, getWaitTime());
        } else {
          LOG.error("Failed to connect to OM node: {}. Attempted {} failovers.",
              omNodeId, failovers);
          return RetryAction.FAIL;
        }
      }
  ...
  protected synchronized void setOmNodeAddress(InetSocketAddress map) {
    this.omNodeAddress = map;
  }
