...
/**
 * Tests key deletion in Multi OM HA setup.
 */
public class TestMultiOzoneManagerHAKeyDeletion
    extends TestMultiOzoneManagerHA {

  @Test
  public void testKeyDeletion() throws Exception {
    // One OzoneBucket for each OM HA
    ...
    for (int i = 0; i < getNumOfOmClusters(); i++) {
      OzoneBucket ozoneBucket = ozoneBuckets.get(i);
      ...
      OzoneManager ozoneManager = getCluster().getOMLeader(i);

      KeyDeletingService keyDeletingService =
          (KeyDeletingService) ozoneManager.getKeyManager()
              .getDeletingService();
      ...
      getCluster().getOzoneManagersList(i).forEach((om) -> {
        try {
          GenericTestUtils.waitFor(() -> {
          ...
          fail("TestMultiOzoneManagerHAKeyDeletion failed");
        }
      });
    }
  }
