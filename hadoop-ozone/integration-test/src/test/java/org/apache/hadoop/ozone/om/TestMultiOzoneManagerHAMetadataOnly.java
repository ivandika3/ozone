 ...
 */
public class TestMultiOzoneManagerHAMetadataOnly extends
    TestMultiOzoneManagerHA {

  private OzoneVolume createAndCheckVolume(int omClusterIndex,
  ...
  public void testAllVolumeOperations() throws Exception {
    for (int i = 0; i < getNumOfOmClusters(); i++) {
      ...
      createAndCheckVolume(i, volumeName);
  ...
  public void testAllBucketOperations() throws Exception {
    for (int i = 0; i < getNumOfOmClusters(); i++) {
      ...
      OzoneVolume retVolume = createAndCheckVolume(i, volumeName);
  ...
  public void testOMProxyProviderInitialization() throws Exception {
    for (int i = 0; i < getNumOfOmClusters(); i++) {
      OzoneClient rpcClient = getCluster().getRpcClient(i);
      ...
      Assert.assertEquals(getNumOfOMsPerCluster(), omProxies.size());

      for (int j = 0; j < getNumOfOMsPerCluster(); j++) {
        InetSocketAddress omRpcServerAddr =
            getCluster().getOzoneManager(i, j).getOmRpcServerAddr();
        ...
        Assert.assertTrue("There is no OM Client Proxy corresponding to OM " +
                "node " + getCluster().getOzoneManager(i, j).getOMNodeId(),
            omClientProxyExists);
      }
    }
  }
  ...
  public void testOMProxyProviderFailoverOnConnectionFailure()
    ...
    for (int i = 0; i < getNumOfOmClusters(); i++) {
      ...
      String firstProxyNodeId = omFailoverProxyProvider
      ...
      getCluster().stopOzoneManager(i, firstProxyNodeId);
  ...
  public void testOMRetryProxy() throws Exception {
    ...
    for (int i = 0; i < getNumOfOmClusters(); i++) {
      // Stop all the OMs.
      for (int j = 0; j < getNumOfOMsPerCluster(); j++) {
        getCluster().stopOzoneManager(i, j);
      }
  ...
  public void testReadRequest() throws Exception {
    for (int i = 0; i < getNumOfOmClusters(); i++) {
      ...
      for (int j = 0; j < getNumOfOMsPerCluster(); j++) {
        // Failover OMFailoverProxyProvider to OM at index j
        OzoneManager ozoneManager = getCluster().getOzoneManager(i, j);

        // Get the ObjectStore and FailoverProxyProvider for OM at index j
        final ObjectStore store = OzoneClientFactory.getRpcClient(
            getOmServiceId(i), getConf()).getObjectStore();
  ...
  public void testListVolumes() throws Exception {
    for (int i = 0; i < getNumOfOmClusters(); i++) {
      ...
      validateVolumesList(i, userName, expectedVolumes);
  ...
  public void testJMXMetrics() throws Exception {
    for (int i = 0; i < getNumOfOmClusters(); i++) {
      ...
      OzoneManagerRatisServer ratisServer =
          getCluster().getOzoneManager(i, 0).getOmRatisServer();
  ...
  public void testOMRetryCache() throws Exception {
    for (int i = 0; i < getNumOfOmClusters(); i++) {
      ...
      OzoneManagerRatisServer ozoneManagerRatisServer =
          getCluster().getOzoneManager(i, currentLeaderNodeId)
              .getOmRatisServer();
          ...
          OMRequest.newBuilder().setCreateVolumeRequest(
                  CreateVolumeRequest.newBuilder().setVolumeInfo(
                          VolumeInfo.newBuilder().setOwnerName(userName)
                              .setAdminName(userName).setVolume(volumeName)
                              .build())
  ...
  private void validateVolumesList(int omClusterIndex, String userName,
                                   Set<String> expectedVolumes)
      throws Exception {
    ObjectStore objectStore = getObjectStore(omClusterIndex);
